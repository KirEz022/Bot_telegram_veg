...
async def make_changes_command(message: types.Message):
    ...
    await bot.send_message(message.from_user.id,'Бот к Вашим услугам',  reply_markup=veg_admin_keyboards.button_l4)
...
async def load_recipe(message: types.Message, state: FSMContext):
    if message.from_user.id == ID:
        ...
        await sqlite_dessert.sql_add_command(state)
        await state.finish()
...
async def delete_item(message: types.Message):
    if message.from_user.id == ID:
        read = await sqlite_dessert.sql_read2()
        for ret in read:
...
def register_handlers_admin(dp: Dispatcher):
    ...
    dp.register_message_handler(make_changes_command, commands=['Модератор4'], is_chat_admin=True)
